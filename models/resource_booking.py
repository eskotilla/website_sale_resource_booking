...
class ResourceBooking(models.Model):
    ...
    prereserved_name = fields.Char(
        string="Prereserved Name",
    )
    prereserved_email = fields.Char(
        string="Prereserved E-mail",
    )
    ...
    def _confirm_prereservation(self):
        ...
        for booking in affected:
            ...
            resource_partner_ids = (
                resource_partners - self.env.user.partner_id
            ).ids
            if resource_partner_ids:
                booking._message_auto_subscribe_notify(
                    resource_partner_ids,
